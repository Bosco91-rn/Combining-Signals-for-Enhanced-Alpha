...
get_ipython().system('{sys.executable} -m pip install -U setuptools')
...
# Compared to the 5 days, target_4 and target_3 have the lowest correlation compared to the 5 days. Target_1 and target_2 have the strongest correlation compared to the 5 days. Normally when we have numbers close to the value 0 means low correlation.
...
def train_valid_test_split(all_x, all_y, train_size, valid_size, test_size):
    ...
    def split_by_index(df, index_level, sets):
        set_indices = split_into_sets(all_x.index.levels[index_level], sets)
        return [df.loc[indices[0]:indices[-1]] for indices in set_indices]
    
    x_train, x_valid, x_test = split_by_index(all_x, 0, [train_size, valid_size, test_size])
    y_train, y_valid, y_test = split_by_index(all_y, 0, [train_size, valid_size, test_size])
...
# *#TODO: Put Answer In this Cell*
# In the case of the "entropy" criterion used in the model, the goal is to maximize the reduction in entropy within the tree's leaves. When the first split is made on the Momentum_1YR feature, it aims to find the best initial separation to reduce the overall entropy of the data. However, the splitting process doesn't stop after the first split and continues to split the tree's leaves based on other features to further reduce entropy and achieve purer leaves. To resume, the first split is made on the 'Momentum_1YR' feature, 'dispersion_20d' plays an important role after that, contributing to further entropy reduction. This leads to a higher overall importance for 'dispersion_20d' compared to 'Momentum_1YR'.
# 
# So, even though the first split occurs on 'Momentum_1YR' only, feature importance should be determined by considering the entire decision tree and how the features influence entropy reduction throughout all the tree's splits, not only at the beginning.
...
# *#TODO: Put Answer In this Cell*
# 1)Considering our accuracy vs tree size graph, is interesting to notice that entropy of nodes of our model at the beginning is higher, it drops and after that continues to be around 1.0, meanwhile accuracy is dropping a bit and get higher after being quite stable at the end, then is a similar trend. It could means that entropy is quite correlated to our accuracy of our model.
# 2) The graph doesn't indicate signs of overfitting or underfitting. Overfitting occurs when the model performs well on the training data but badly on validation or test set. In this case, the validation accuracy remains stable and doesn't show a significant decrease or divergence. This suggests that the model is not overfitting.
# Underfitting occurs when the model performs poorly on both training and validation data. In this case, training and validation accuracies remain relatively stable when the number of trees increases. This suggests that the model is not underfitting.
# The model then reached a stable and consistent performance on both the training and validation sets, indicating a reasonable fit to the data.
...
def non_overlapping_samples(x, y, n_skip_samples, start_i=0):
    ...
    # TODO: Implement
    x_index = x.index.levels[0][start_i::n_skip_samples+1].tolist()
    y_index = y.index.levels[0][start_i::n_skip_samples+1].tolist()
    
    non_overlapping_x = x.loc[x_index]
    non_overlapping_y = y.loc[x_index]
...
def calculate_oob_score(classifiers):
    ...
    for clf in classifiers:
        # Check if the classifier is a regressor
        if isinstance(clf, RandomForestRegressor):
            # Calculate the out-of-bag score for the regressor
            oob_score = clf.oob_score_
        else:
            # Create a list to store the out-of-bag predictions
            oob_predictions = []

            # Loop through each training instance
            for i, (X_train, y_train) in enumerate(clf.oob_decision_function_):
                # Get the excluded sample from training
                X_oob = X_train[i]
                y_oob = y_train[i]

                # Make a prediction on the excluded sample
                y_pred = clf.predict([X_oob])[0]

                # Add the prediction to the out-of-bag predictions
                oob_predictions.append(y_pred)

            # Calculate out-of-bag score for the classifier
            oob_score = mean_squared_error(y_oob, oob_predictions)  # or r2_score(y_oob, oob_predictions) for regression

        # Add out-of-bag score to the list of out-of-bag scores
        oob_scores.append(oob_score)

    # Calculate mean of the out-of-bag scores
    mean_oob_score = sum(oob_scores) / len(oob_scores)

    return mean_oob_score
...
def non_overlapping_estimators(x, y, classifiers, n_skip_samples):
    ...
    for ii in range(len(classifiers)):
        x_ov, y_ov = non_overlapping_samples(x, y, n_skip_samples, ii)
        lista.append(classifiers[ii].fit(x_ov, y_ov))   
    return lista
